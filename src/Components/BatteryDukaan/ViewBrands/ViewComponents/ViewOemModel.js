		...
		oemModelTable = props.OemModel.data.map((details, index) =>{
			return (
				<tr key={index}>
					...
					<td>{details["FUEL TYPE"]}</td>
					<td>
						{" "}
						<a href={details["Oem-Model-Images"]} target="_blank">
							{details["Oem-Model-Images"]}
						</a>
					</td>
					<td>{details["Oem-Model-position"]}</td>
					<td>{details["OEM-Brand Name"]}</td>
					<td>
						<svg
